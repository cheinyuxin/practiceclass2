...
```{r ans1}
library(readr)
sum(gsHW2ws$'ans1'==1, na.rm=TRUE)/45*100
```

## 1.2
使用`stringr::str_which()`來找出gsHW2ws中所有變數名稱包含的"ans"的欄位位置。
```{r ans12}
colnames(gsHW2ws[,1:5])
```
...
library(lubridate)
library(stringr)
stringr::str_c(allBankData$'年月',"/01")->dateTime
lubridate::ymd(dateTime)+lubridate::years(911)->dateTime
#allBankData$年月<-dateTime
```
...
```{r ans31}
library(dplyr)
mutate_at(exData)->data.class(exData)
```
